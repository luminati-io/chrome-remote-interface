...
class Chrome extends EventEmitter {
    ...
    _connect() {
        return new Promise((fulfill, reject) => {
            ...
            this._transport.on('message', (data) => {
                ...
                this._handleMessage(message, data.length);
            });
    ...
    _handleMessage(message, size) {
        ...
        if (message.id) {
            ...
            const duration = Date.now()-this._callbacks_start_ts[message.id];
            if (message.duration)
                this.emit('cmd_rtt', duration-message.duration);
            const ctx = this._callbacks_req_ctx[message.id];
            if (ctx)
            {
                this.emit('cmd_rx_size', {method: ctx.method, size},
                    ctx.sessionId);
            }
            ...
            delete this._callbacks_start_ts[message.id];
            delete this._callbacks_req_ctx[message.id];
        ...
        // event
            ...
            this.emit('cmd_rx_size', {method, size}, sessionId);
        }
    }
    ...
    _enqueueCommand(method, params, sessionId, callback) {
        ...
        this._send(JSON.stringify(message), (err) => {
            if (err) {
                ...
                this._callbacks_start_ts[id] = Date.now();
            }
        });
    }
